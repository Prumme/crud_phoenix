defmodule AurelWeb.ContactController do
  ...
  def new(conn, _params) do
    changeset = DB.change_contact(%Contact{})
    render(conn, "new.html", changeset: changeset)
  end

  def create(conn, %{"contact" => contact_params}) do
    case DB.create_contact(contact_params) do
      {:ok, contact} ->
        conn
  ...
  def update(conn, %{"id" => id, "contact" => contact_params}) do
    ...
    case DB.update_contact(contact, contact_params) do
      {:ok, contact} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    contact = DB.get_contact!(id)
    {:ok, _contact} = DB.delete_contact(contact)
